...
# 导入已经编写好的模块
from cs336_data.filter_email import run_mask_emails
from cs336_data.filter_ip import run_mask_ips
from cs336_data.filter_phone_number import run_mask_phone_numbers
from cs336_data.quality_rules import run_gopher_quality_filter
...
def extract_negative_samples_from_warc(
    ...
    Args:
        ...
        target_count: 目标样本数量，尽量和正样本数量一致
        random_seed: 随机种子
        ...
        try:
            with gzip.open(warc_path, 'rb') as f:
                for record in tqdm(ArchiveIterator(f), desc=f"Processing WARC {warc_idx}"):
                    # ArchiveIterator 自动解析 WARC 文件格式，逐条返回记录对象（record）
                    total_records += 1
...
def main():
    datasets_dir = r"D:\Desktop\Stanford-CS336\Assignment-4\local-shared-data\CC"
    ...
    # 输出文件路径
    output_path = r"D:\Desktop\Stanford-CS336\Assignment-4\local-shared-data\quality_classify_dataset\wiki_negative_samples.txt"
