...
if __name__ == "__main__":
    ...
    try:
        ...
        test_text = "Hello world! This is a test of the BPE tokenizer. 你好，世界！12345 测试中文和英文混合。"
    ...
    # 特殊 token 测试
    special_test = "Hello world<|endoftext|>This is after special token. 你好<|endoftext|>世界"
    print("\n特殊 token 测试:")
    ...
    if 'lines' in locals() and lines:
        test_texts = lines[:2000]  # 使用较多真实数据
    ...
    print(f"\n批量测试 ({len(test_texts)} 条文本 × {num_runs} 次运行):")
    for run in range(num_runs):
        start_time = time.perf_counter()
        batch_encoded = tokenizer.encode_batch(test_texts)  # 使用批量方法
        end_time = time.perf_counter()
        ...
        print(f"  Run {run+1}: {run_tokens:,} tokens in {run_time:.3f}s → {run_tokens/run_time:,.0f} tokens/s")
