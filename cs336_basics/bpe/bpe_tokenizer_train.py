...
def run_train_bpe(
    ...
    vocab: Dict[int, bytes] = {i: bytes([i]) for i in range(
        256)}  # bytes()是将整数转换为字节序列的函数，bytes(3)=bytes([0,0,0])即只传数字就是构造三个0，如果传数字列表bytes([65,66])=b'AB'返回ASCII码，注意范围是0-255
    ...
    for word_str in raw_words:
        ...
        if not word_as_raw_bytes:  # 跳过空字符串（可能由多个连续空格产生）
            continue
        unicode_sequences.append([_BYTES_TO_UNICODE_MAP[byte_val] for byte_val in
                                  word_as_raw_bytes])  # 将单词映射为unicode字符，并添加到unicode_sequences中
